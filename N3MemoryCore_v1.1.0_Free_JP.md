...
Pro 版指示書（`N3MemoryCore_v1.1.0_Pro_*_Complete.md`）を Claude Code に読み込ませて「この指示書に従って N3MemoryCore を実装してください」と依頼してください。既存の DB はそのまま引き継がれます。
...
## 6. 自律評価（[N3MC v1.1.0 Evidence Report]）
...
| `TestBuffer` | 保存、空コンテンツ、agent_name付き、完全一致dedup、コードブロック純化 | 保存API |
