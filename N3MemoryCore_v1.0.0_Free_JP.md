...
- **使用許諾**: 本指示書の使用・改変・再配布（商用目的を含む）を自由に許可します。再配布時に N3MemoryCore の名前に触れていただけると嬉しいです。
...
Pro 版指示書（`N3MemoryCore_v1.0.0_Pro_*_Complete.md`）を Claude Code に読み込ませて「この指示書に従って N3MemoryCore を実装してください」と依頼してください。既存の DB はそのまま引き継がれます。
...
> **重要：文字数制限（実装時の設計制約）**
> - Hook 自動保存：1レコードあたり最大 **300 文字**（文末境界で切り詰め）
...
> - **長文の処理**: ユーザーが長文（仕様書、記事、チャットログ等）を貼り付けた場合、そのまま保存してはいけません。全文を読んで理解した上で、重要な事実を1件ずつ短文（約50〜200文字）に要約し、個別の `--buffer` で保存してください。生テキストをそのまま保存すると切り詰められ、情報が失われます。
...
│   │   └── processor.py     # 処理層: 埋め込み生成・ランキング計算・テキスト純化
...
| `local_id` | `config.json` | 初回起動時 (UUIDv4) | **エージェント / インストール** | エージェントの UUIDv4 識別子。マルチエージェント時は各エージェント固有の UUID を指定。検索バイアス `b_local` に使用 |
...
CREATE TABLE memories (
    ...
    agent_id  TEXT              -- レコードを書いたAIエージェントの識別子（例：`"claude-code"`）。v1.1以前または指定なしの場合は NULL
...
| `POST` | `/buffer` | `--buffer` | `{"content": str, "agent_id": str（オプション）}` を受け取り保存 |
| `POST` | `/search` | `--search` | `{"query": str}` を受け取り結果を返す |
| `POST` | `/repair` | `--repair` | 未インデックスレコードを修復 |
| `GET` | `/list` | `--list` | 全レコードの一覧を返す（各レコードに `agent_id` を含む） |
...
stdin から JSON を読み取り、`message`（または `prompt`）と `last_assistant_message` フィールドを使用する。単一プロセス内で HTTP リクエスト経由により全 UserPromptSubmit 処理を実行する：`--repair` → `--buffer`（Claude の回答を保存）→ `--search` → `--buffer`（ユーザー発言を保存）。`n3mc_hook.py` から呼び出される。AI が手動で実行する必要はない。このフックで保存される全レコードには `agent_id = "claude-code"` が自動タグ付けされる。
...
- **【自動化】修復・検索・会話の自動保存**: `UserPromptSubmit` フック（`n3mc_hook.py`）は `--hook-submit` を呼び出し、単一プロセス内で全処理を実行する：`--repair`（未ベクトル化修復）、`--buffer`（Claude の直前の回答を `[claude]` プレフィックス付きで自動保存、3文字未満スキップ、300文字以内の文末境界で切り詰め）、`--search`（記憶取得）、`--buffer`（ユーザー発言を `[user]` プレフィックス付きで自動保存、10文字未満や定型応答はスキップ、300文字以内の文末境界で切り詰め）。これにより**全ての会話ターン**（ユーザー・Claude 双方）が記録される。AI が手動で実行する必要はない。Claude Code が画像+テキストのプロンプトを渡す場合、`prompt` フィールドが JSON 配列になる場合がある。`_extract_text()` は `type=="text"` の部分のみを抽出し画像データを破棄する。結果が空（画像のみのプロンプト）の場合はバッファと検索の両方をスキップする。
...
- **【自動化】Claude の回答の自動保存**: `Stop` フック（`n3mc_stop_hook.py`）により、Claude の最終回答を自動保存する（3文字未満はスキップ、300文字以内の文末境界で切り詰め）。同時に `--stop`（CLAUDE.md の @import 設定）を実行する。1ターンあたりの合計サブプロセス数: `UserPromptSubmit` × 1（`--hook-submit`）+ `Stop` × 2（buffer + stop）= 3回。
...
- **【AI行動指針】全自動保存**: 全会話はフック（UserPromptSubmit / Stop）により自動保存される。Claude が自分で判断して `--buffer` を手動呼び出しする必要はない。注: 自動保存フックには意図的な最小限のノイズフィルタが存在する——3文字未満の回答、10文字未満のユーザー発言、および定型応答はDBへの汚染防止のためスキップされる。
...
- **【カスタマイズ】言語ローカライズ**: `n3mc_hook.py` のスキップパターン（`_SKIP_PATTERNS`）はデフォルトで英語の定型応答に設定されている。自国語の短文フィルタに最適化する場合は、Claude Code に「スキップパターンを○○語に最適化して」と依頼すること。
- **【実装仕様】重複排除**:
  - サーバー起動中: cos_sim ≥ `dedup_threshold`（0.95）または文字列完全一致なら保存をスキップ。
  - サーバー停止中: 重複チェックをスキップし、**文字列完全一致のみ**で排除した上で保存を続行せよ（保存ブロック禁止）。このフォールバック（`_buffer_direct()`）は埋め込みベクトルを生成せずに直接 DB へ書き込む。欠損した vec インデックスエントリは次回の `--repair` 呼び出しで修復される。
...
## 6. 自律評価（[N3MC v1.0.0 Evidence Report]）
実装完了後、以下のテストを自律解決し、満点（⭐⭐⭐⭐⭐）で報告せよ。

1. **常駐速度 & プロセス管理**: `--search` の応答時間を計測し記録せよ（達成目標: GPU 環境で 0.7s 前後、CPU 環境では 2.0s 以内を許容）。PIDファイルの生成・削除・再起動が正常に機能することを確認せよ。
...
2. **強制終了テスト（Durabilityの証明）**: データを1件 `--buffer` で保存直後にプロセスを強制終了（Ctrl+C）し、再起動後に `--list` でそのレコードがDBに残留していることを物理的に証明せよ。`--list` の出力フォーマットは以下の形式とする（1件1行、タブ区切り）:
   ...
   ```
   [UUIDv7]  [timestamp]  [agent_id]  [先頭80文字のcontent]
   ```
...
4. **架空設定テスト（創造的架空設定）**: コードブロックを含まない架空の設定（例: キャラクター名・世界観・固有名詞）を含むテキストを保存後、`--search` で取得し、**保存したテキストの全フィールドが一字一句変化なく復元できる**ことを合格基準とする。なお、コードブロックを含む場合は §5 の物理的純化により `[code omitted]` に置換されるため復元結果が異なる（これは仕様上の正常動作）。
   - 日本語版の例: 「ソラニア（架空の浮遊都市）」
...
9. **ノイズフィルタテスト**: 以下の入力が正しくフィルタされることを確認せよ。
    - 2文字の文字列を `[claude]` プレフィックスで `--hook-submit` に渡し、**保存がスキップされる**こと（3文字未満フィルタ）。
    - 5文字の文字列を `[user]` プレフィックスで `--hook-submit` に渡し、**保存がスキップされる**こと（10文字未満フィルタ）。
    - 定型語（例: `ok`、`yes`）を `[user]` として渡し、**保存がスキップされる**こと。
    - 上記すべてのケースで、`--buffer` を直接呼び出した場合は**フィルタを適用せず保存される**ことを確認（手動保存はフィルタの対象外）。
...
10. **全自動保存テスト（Fully Automatic Saving の検証）**: 以下の手順でフックによる全自動保存が正常に機能していることを確認せよ。
    ...
    4. **合格基準**: 会話ターンごとにユーザー発言・Claude 回答が各1件ずつ保存されている（ノイズフィルタでスキップされる極短文・定型応答を除く）。Claude が `--buffer` を手動呼び出しした形跡がないこと。
...
└── tests/
    ...
    ├── test_processor.py    # Layer 2: ランキング数学、純化、Refresh
    ├── test_api.py          # Layer 3: FastAPIエンドポイントテスト（TestClient）
    └── test_hooks.py        # Layer 4: フック統合（切り詰め、画像除去、スキップパターン）
```
...
| `TestPurification` | コードブロック置換、インラインコード保持、複数ブロック、ブロック無し | 物理的純化 |
...
| `TestBuffer` | 保存、空コンテンツ、agent_id付き、完全一致dedup、コードブロック純化 | 保存API |
...
| テストクラス | テスト内容 | カバレッジ |
|---|---|---|
| `TestTruncateAtSentence` | 短文不変、ピリオド切断、感嘆符切断、ハードカット | 文末切り詰め |
| `TestStripImages` | 画像なし不変、base64除去、画像のみ空文字、非JSON通過 | 画像ペイロード除去 |
| `TestExtractText` | プレーン文字列、マルチモーダルJSON、画像のみ空、空入力 | マルチモーダル抽出 |
| `TestSkipPatterns` | 定型語スキップ、意味ある文は通過、Claude/Userの文字数フィルタ | ノイズフィルタ |
...
- GPU では約 0.02〜0.05 秒/回に短縮される可能性がありますが、小型モデルでは GPU 転送オーバーヘッドにより効果が出ない場合があります
- ユーザーの 1 プロンプトあたり 3 回の embedding 計算が発生するため、GPU 化による遅延削減効果は大きい
