...
| ファイル | 内容 |
|---|---|
| `N3MemoryCore_v1.1.0_Free_JP.md` | Free版（日本語） |
| `N3MemoryCore_v1.1.0_Free_EN.md` | Free版（英語） |
...
| `local_id` (agent_id) | `config.json` | 初回起動時 (UUIDv4) | **エージェント / インストール** | エージェントの UUIDv4 識別子。マルチエージェント時は各エージェント固有の UUID を指定 |
